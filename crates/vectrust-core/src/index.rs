...
impl StorageBackend for DummyStorage {
    ...
    async fn create_index(&mut self, _config: &CreateIndexConfig) -> Result<()> {
        Ok(())
    }
    async fn get_item(&self, _id: &uuid::Uuid) -> Result<Option<VectorItem>> {
        Ok(None)
    }
    async fn insert_item(&mut self, _item: &VectorItem) -> Result<()> {
        Ok(())
    }
    async fn update_item(&mut self, _item: &VectorItem) -> Result<()> {
        Ok(())
    }
    async fn delete_item(&mut self, _id: &uuid::Uuid) -> Result<()> {
        Ok(())
    }
    async fn list_items(&self, _options: Option<ListOptions>) -> Result<Vec<VectorItem>> {
        Ok(Vec::new())
    }
    async fn query_items(&self, _query: &Query) -> Result<Vec<QueryResult>> {
        Ok(Vec::new())
    }
    async fn begin_transaction(&mut self) -> Result<()> {
        Ok(())
